<?php 
	...
	if(isset($_POST['submit'])){
		...
		if(empty($username)||empty($password)||empty($repasswd)){
			header("Location: ../eshop/login.php?error=emptyfields&username=".$username);
			exit();
		}else if($password !== $repasswd){
			header("Location: ../eshop/login.php?error=passwordcheck&username=".$username);
			exit();
		}else{
			$conn = new sqlConnect();
			...
			if($password === $db_passwd){
				...
				header("refresh: 3;URL= ../eshop/main.php");
				exit();
			}else{
				echo "fail to login";
			}
